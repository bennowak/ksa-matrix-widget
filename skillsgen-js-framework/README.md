...
## Proposed Solution
The KSA "Matrix Widget" is a small web app that allows users to upload a csv file and get back a JSON object that can be consumed by the `skillsgen-js-framework` is driven primarily by a simple CSV file formate that is used to generate a valid JSON file for consumption by API's that can either store the ksa-profile, or for dynamic rendering on a WebSite.
